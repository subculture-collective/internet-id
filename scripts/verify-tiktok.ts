...
async function main() {
  const [tiktokUrlOrId, registryAddress] = process.argv.slice(2);
  if (!tiktokUrlOrId || !registryAddress) {
    console.error("Usage: npm run verify:tiktok -- <tiktokUrlOrId> <registryAddress>");
    process.exit(1);
  }

  const tiktokId = extractTikTokId(tiktokUrlOrId);
  if (!tiktokId) {
    console.error("Could not extract TikTok ID");
    process.exit(1);
  }
  ...
  if (!timestamp || contentHash === ethers.ZeroHash) {
    console.error("FAIL: No binding found on-chain for this TikTok ID");
    process.exit(1);
  }
