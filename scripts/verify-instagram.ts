...
async function main() {
  const [instagramUrlOrId, registryAddress] = process.argv.slice(2);
  if (!instagramUrlOrId || !registryAddress) {
    console.error("Usage: npm run verify:instagram -- <instagramUrlOrId> <registryAddress>");
    process.exit(1);
  }

  const instagramId = extractInstagramId(instagramUrlOrId);
  if (!instagramId) {
    console.error("Could not extract Instagram ID");
    process.exit(1);
  }
  ...
  if (!timestamp || contentHash === ethers.ZeroHash) {
    console.error("FAIL: No binding found on-chain for this Instagram ID");
    process.exit(1);
  }
