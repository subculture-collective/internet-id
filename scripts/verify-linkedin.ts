...
async function main() {
  const [linkedinUrlOrId, registryAddress] = process.argv.slice(2);
  if (!linkedinUrlOrId || !registryAddress) {
    console.error("Usage: npm run verify:linkedin -- <linkedinUrlOrId> <registryAddress>");
    process.exit(1);
  }

  const linkedinId = extractLinkedInId(linkedinUrlOrId);
  if (!linkedinId) {
    console.error("Could not extract LinkedIn ID");
    process.exit(1);
  }
  ...
  if (!timestamp || contentHash === ethers.ZeroHash) {
    console.error("FAIL: No binding found on-chain for this LinkedIn ID");
    process.exit(1);
  }
