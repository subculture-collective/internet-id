...
async function main() {
  const [discordUrlOrId, registryAddress] = process.argv.slice(2);
  if (!discordUrlOrId || !registryAddress) {
    console.error("Usage: npm run verify:discord -- <discordUrlOrId> <registryAddress>");
    process.exit(1);
  }

  const discordId = extractDiscordId(discordUrlOrId);
  if (!discordId) {
    console.error("Could not extract Discord ID");
    process.exit(1);
  }
  ...
  if (!timestamp || contentHash === ethers.ZeroHash) {
    console.error("FAIL: No binding found on-chain for this Discord ID");
    process.exit(1);
  }
